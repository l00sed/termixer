    ...
    I: Source<Item = f32>,
{
    ...
    fn read_delayed(&self, buffer_idx: usize, delay_samples: f32) -> f32 {
        ...
        let read_pos =
            (self.write_pos as f32 - delay_samples + buffer.len() as f32) % buffer.len() as f32;
    ...
    I: Source<Item = f32>,
{
    pub fn new(input: I, room_size: f32, decay: f32, mix: f32) -> Self {
        ...
        Self {
            ...
            allpass_buffers: [vec![0.0; allpass_delays[0]], vec![0.0; allpass_delays[1]]],
    ...
    I: Source<Item = f32>,
{
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        for i in 0..2 {
            ...
            self.allpass_positions[i] =
                (self.allpass_positions[i] + 1) % self.allpass_buffers[i].len();
